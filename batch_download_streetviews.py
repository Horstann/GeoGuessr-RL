...
# =====================================================================
# Graph processing & image downloading
# =====================================================================
...
async def prepare_batch(
    ...
    for graph_file in graph_files:
        ...
        if processed_path.exists():
            ...
            raise FileExistsError(f"The source graph cannot be archived because this file already exists: {processed_path}")
...
async def finish_upload(batch, upload_task, known_files):
    ...
    try:
        try:
            ...
            # Cancelling to_thread only cancels the awaitable; its worker thread keeps running. Wait for it before deleting the staged files.
        ...
        if existing_paths:
            ...
            raise FileExistsError(f"The upload succeeded, but its source graphs could not be archived because these files already exist: {path_list}")

        for source_graph, processed_path in zip(batch.source_graphs, processed_paths):
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
        ...
        for batch_start in range(0, len(source_graphs), GRAPH_BATCH_SIZE):
            ...
            if uploading_batch is not None:
                print(f"Uploading {len(uploading_batch.source_graphs)} graph(s) in the background.")
                upload_task = asyncio.create_task(
                    asyncio.to_thread(commit_batch, uploading_batch)
                )
            else: upload_task = None
